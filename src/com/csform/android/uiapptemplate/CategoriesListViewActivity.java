...
public class CategoriesListViewActivity extends ActionBarActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Scan Option
		if (extras != null
				&& extras.containsKey(LIST_VIEW_CATEGORY_SCAN)) {
			
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB_MR1) {
				category = extras.getString(LIST_VIEW_CATEGORY_SCAN,
						LIST_VIEW_UNIVERSAL);
			} else {
				category = extras.getString(LIST_VIEW_CATEGORY_SCAN);
		...
		//Test scan option
		if(category.equals(LIST_VIEW_OPTION_SCAN))	{
			// Su dung asyns task để làm phần Scan
		...
		} else 	if (!category.equals(LIST_VIEW_OPTION_APPEARANCE_ANIMIATIONS))
					setUpListView();
				else
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if (category.equals(LIST_VIEW_SOCIAL)) {
			...
			// nếu category khác Appearance animiation thì truyền tham số listViewCategory = category ở onCreate
			if (!listViewCategory
					.equals(LIST_VIEW_OPTION_APPEARANCE_ANIMIATIONS))
				intent = getCategoryIntent(listViewCategory);
			else
				// nếu là Animiation thì truyền vào category là Child đang được click
